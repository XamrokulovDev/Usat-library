...
const CreateBooks: React.FC = () => {
  ...
  const [selectedBookForDelete, setSelectedBookForDelete] = useState<BookType | null>(null)
  ...
  const handleEditBook = (book: BookType): void => {
    ...
    if (book.image) {
      const imageUrl = `${import.meta.env.VITE_API}${book.image.url.startsWith("/") ? "" : "/"}${book.image.url}`
  ...
  const handleDeleteOk = async () => {
    ...
    try {
      ...
      await axios.delete(`${import.meta.env.VITE_API}/api/books/${selectedBookForDelete.id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "X-permission": permissionIds[0],
        },
  ...
  return (
    <>
                ...
                <div className="flex items-center justify-center w-full">
                  ...
                  >
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">PNG, JPG, JPEG (MAX. 100MB)</p>
                    </div>
                    <input
                      ref={fileInputRef}
                      id="image-upload"
                  ...
                  {filteredBooks.map((item: BookType, index: number) => (
                    <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      ...
                      <td className="px-6 py-2 whitespace-nowrap text-center text-blue-500 dark:text-blue-500 underline cursor-pointer">
                        <button onClick={() => setSelectedBook(item)}>ko'rish</button>
                      </td>
      ...
      <Modal
        ...
        cancelText="Yo'q"
      >
        <p>{selectedBookForDelete ? `"${selectedBookForDelete.name}" kitobini o'chirmoqchimisiz?` : ""}</p>
      </Modal>
    </>
  )
}
