...
const CreateBooks: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    const data: PreviewDataType = {
      name: bookName,
      auther_name: auther.find((a) => a.id.toString() === selectedAutherId)?.name || "",
    ...
    try {
      ...
      await axios.post(
        `${import.meta.env.VITE_API}/api/books`,
        {
          ...
          auther_id: previewData.auther_id,
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
        ...
        {fetchLoading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-6 mt-15">
            ...
            >
              {submitLoading ? "Tekshirilmoqda..." : "Ma'lumotlarni Tekshirish"}
            </button>
          </form>
        )}
      </div>

      {/* Preview section - only shows when showPreview is true */}
      {showPreview && previewData && (
        <div
          id="preview-section"
          className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 mt-10"
        >
          <div className="flex items-center gap-2 mb-4">
            <BookPlus className="w-6 h-6 text-blue-600 dark:text-blue-400" />
            <h3 className="text-xl font-semibold text-gray-800 dark:text-white/90">Ma'lumotlarni tasdiqlang</h3>
          </div>

          <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 space-y-3 max-h-96 overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Kitob nomi:</span>
                <p className="text-gray-900 dark:text-white font-medium">{previewData.name}</p>
              </div>
              <div>
                <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Muallif:</span>
                <p className="text-gray-900 dark:text-white font-medium">{previewData.auther_name}</p>
              </div>
              <div>
                <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Chiqarilgan yil:</span>
                <p className="text-gray-900 dark:text-white font-medium">{previewData.year}</p>
              </div>
              <div>
                <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Bet soni:</span>
                <p className="text-gray-900 dark:text-white font-medium">{previewData.page}</p>
              </div>
              <div>
                <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Kitob soni:</span>
                <p className="text-gray-900 dark:text-white font-medium">{previewData.books}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-3 mt-6">
            <button
              ...
              disabled={submitLoading}
              className="flex-1 bg-gray-600 hover:bg-gray-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white rounded-lg shadow-md transition px-6 py-3 font-medium"
            >
              Bekor qilish
            </button>
            <button
              ...
              disabled={submitLoading}
              className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white rounded-lg shadow-md transition px-6 py-3 font-medium"
            >
              {submitLoading ? "Saqlanmoqda..." : "Tasdiqlash"}
            </button>
          </div>
        </div>
      )}
