...
export default function WeeklyOrderChart() {
  ...
  const processOrderData = (orders: OrderType[]): WeeklyData[] => {
    ...
    orders.forEach((order) => {
      ...
      const day = orderDate.getDay() // Yakshanba=0, Dushanba=1, ... Shanba=6

      // Bizning tartib: Dushanba(0), ... Yakshanba(6)
      const mappedDay = day === 0 ? 6 : day - 1
      weekCounts[mappedDay]++
    })
  ...
  const totalOrders = weeklyData.reduce((sum, item) => sum + item.count, 0)
  const highestDay = weeklyData.reduce((max, item) => (item.count > max.count ? item : max), weeklyData[0])
  const averageOrders = Math.round(totalOrders / weeklyData.length)

  const todayIdx = (new Date().getDay() === 0 ? 6 : new Date().getDay() - 1)
  const todayCount = weeklyData[todayIdx]?.count || 0

  return (
    <div className="max-w-full overflow-x-auto custom-scrollbar">
      <div id="weeklyOrderChart" className="min-w-[800px]">
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        ...
        <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded-lg">
          ...
          <div className="text-lg font-bold text-green-700 dark:text-green-300">
            {highestDay.day} ({highestDay.count} ta)
          </div>
        </div>
